...
function parseCommandFile(content, filename) {
  try {
    ...
    const extract = (key) => {
      ...
      // ["'`]   -> start quote (any type)
      // (.*?)   -> capture group (non-greedy)
      // ["'`]   -> end quote (same type ideally, but regex here is simple)
      // accepted slight inaccuracy in quote matching for simplicity/robustness against mixed quotes in huge files
    ...
    // Fallback: If no name property found, but file is in commands dir, use filename
    if (!name) {
    ...
    const description = extract("description") || generateDescription(name);
    const category = extract("category") || "general";
    const usage = extract("usage") || name; // Default to name only, no prefix
