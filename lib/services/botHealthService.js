...
class BotHealthService extends EventEmitter {
  ...
  async parseLogForStateChange(deploymentId, logLine) {
    ...
    if (
      /message received|received message|📝 Command used|handling command|\[auto-react\]/i.test(
        log,
      )
    ) {
  ...
  async performHealthCheck(deploymentId) {
    try {
      ...
      // Note: We no longer auto-correct to 'online' based on uptime alone.
      // Status remains 'starting' or 'awaiting_pairing' until log patterns or activity are detected.
