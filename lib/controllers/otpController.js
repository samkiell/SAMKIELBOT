...
const verifyOTPCode = async (req, res) => {
  try {
    ...
    // Check maximum attempts (Max 2)
    if (user.otp.attempts >= 2) {
      return errorResponse(
        res,
    ...
    if (!isValid) {
      ...
      return errorResponse(
        res,
        `Invalid OTP. ${2 - user.otp.attempts} attempts remaining.`,
        400,
      );
    }
    ...
    if (user.referredBy && !user.referralRewardClaimed) {
      ...
      if (referrer) {
        try {
          ...
          console.error("[OTP] Referral processing failed:", refErr);
        }
      }
    }
