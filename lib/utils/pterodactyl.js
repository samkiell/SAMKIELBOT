 ...
 */
const selectBestNode = async (userId) => {
  try {
    const nodes = await getNodes();

    const eligibleNodes = nodes
      .filter((node) => {
        const { maintenance_mode, memory, allocated_resources } =
          node.attributes;

        // Ignore offline or disabled nodes (Requirement 1)
        if (maintenance_mode) return false;

        const totalRam = memory;
        const allocatedRam = allocated_resources
          ? allocated_resources.memory
          : 0;
        const freeRam = totalRam - allocatedRam;
        const usagePercent =
          totalRam > 0 ? (allocatedRam / totalRam) * 100 : 100;

        // Hard safety rules (Requirement 3)
        const hasEnoughFreeRam = freeRam >= 500;
        const underUsageLimit = usagePercent <= 80;

        return hasEnoughFreeRam && underUsageLimit;
      })
      .map((node) => {
        const { memory, allocated_resources } = node.attributes;
        return {
          ...node,
          computed: {
            freeRam:
              memory - (allocated_resources ? allocated_resources.memory : 0),
            allocatedRam: allocated_resources ? allocated_resources.memory : 0,
          },
        };
      });
    ...
    // Sort: Highest free RAM, then Lowest allocated RAM (Requirement 4)
    eligibleNodes.sort((a, b) => {
      if (b.computed.freeRam !== a.computed.freeRam) {
        return b.computed.freeRam - a.computed.freeRam;
      }
      return a.computed.allocatedRam - b.computed.allocatedRam;
    });

    const selectedNode = eligibleNodes[0];

    // Log the selection (Requirement 7)
    console.log(
      `[Deployment] Node ID ${selectedNode.attributes.id} chosen for user ${userId}. Free RAM: ${selectedNode.computed.freeRam}MB`
    );

    return {
      id: selectedNode.attributes.id,
      freeRam: selectedNode.computed.freeRam,
    };
  } catch (error) {
    console.error("Error in selectBestNode:", error.message);
    throw error;
  }
};
...
const findNode = async (ip, userId) => {
  ...
  const bestNode = await selectBestNode(userId);
  if (!bestNode) {
    // This error message is used in failure handling (Requirement 6)
    throw new Error(
      "All nodes are currently at capacity. Please try again later."
    );
  }
...
const createServer = async (deploymentData) => {
  try {
    ...
    const payload = {
      ...
      startup:
        "if [ -d .git ]; then echo 'Updating existing installation...'; git fetch origin $BRANCH && git reset --hard origin/$BRANCH; else echo 'Fresh installation, cloning...'; git clone -b $BRANCH $REPO_URL .; fi; npm install; echo \"Checking settings.js from repo...\"; node index.js",
      environment: {
