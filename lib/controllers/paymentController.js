    ...
    if (req.body.quoteId) {
      ...
      if (quote.provider === "flutterwave") {
        paymentResult = await flutterwaveService.initializePayment({
          ...
          // redirect_url handled in service
        });
      } else {
        // PAYSTACK (Default)
...
exports.createQuote = async (req, res) => {
  try {
    ...
    let convertedSubtotal = basePriceNgn * rate;

    // Enforce minimum amount for Flutterwave (e.g. 1 USD/GBP min)
    if (provider === "flutterwave" && convertedSubtotal < 1) {
      convertedSubtotal = 1.0;
    }

    // Tax (Removed)
    const taxRate = 0;
    const taxAmount = 0;
    const totalAmount = convertedSubtotal;

    // Logic for Paystack NGN Charge (Backwards compatibility)
    let paystackChargeAmount = 0;
    if (provider === "paystack") {
      // NGN Flow: Explicit calculation
      paystackChargeAmount = basePriceNgn;
    }
