...
const createDeployment = async (req, res) => {
  try {
    ...
    const tierLevel = { Free: 0, Starter: 1, Pro: 2, Max: 3 };
    const currentLevel = tierLevel[planName] || 0;
    ...
    if (currentLevel < 1 && packName !== "𝕊𝔸𝕄𝕂𝕀𝔼𝕃 𝔹𝕆𝕋") {
      finalPackName = "𝕊𝔸𝕄𝕂𝕀𝔼𝕃 𝔹𝕆𝕋"; // Force watermark for Free tier
    }

    // 2. Command Mode (Starter+ for Private)
    if (currentLevel < 1 && cleanFeatures.COMMAND_MODE === "private") {
      cleanFeatures.COMMAND_MODE = "public";
    }

    // 3. Auto Status View (Pro+ for no-dl)
    if (currentLevel < 2 && cleanFeatures.AUTO_STATUS_VIEW === "no-dl") {
      cleanFeatures.AUTO_STATUS_VIEW = "on";
    }

    // 4. Auto Reaction (Pro+)
    if (currentLevel < 2 && cleanFeatures.AUTO_REACTION === true) {
      cleanFeatures.AUTO_REACTION = false;
    }
    ...
    const deployment = await Deployment.create({
      ...
      configuration: {
        prefix: finalPrefix,
        ownerName: ownerName || req.user.username, // Fallback to username
...
const processDeployment = async (deploymentId) => {
  ...
  try {
    ...
    modifiedSettings = modifiedSettings
      ...
      .replace(
        /ownerNumber:\s*["'`][^"'`]*["'`]/,
        `ownerNumber: "${deployment.user}"` // In the file it seems it might be string, but let's check original. The original used jidNormalizedUser which is for internal use.
        // The original code was: ownerNumber: "2348087357158" (string)
        // AND user requested: `ownerNumber` input from User DB Profile.
      ) // Wait, previously I saw regex for `jidNormalizedUser`.
      // Let's stick to the previous robust replacement for numbers if possible,
      // BUT `ownerNumber` in the user payload said "2348087357158" (String).
      // Let's fetch the REAL owner number from the User ID attached to deployment.
      .replace(
        /ownerName:\s*["'`][^"'`]*["'`]/,
    ...
    // We need the user's phone number from the DB, not just their ID.
    const deploymentUser = await User.findById(deployment.user);
    if (deploymentUser && deploymentUser.whatsappNumber) {
      modifiedSettings = modifiedSettings.replace(
        /ownerNumber:\s*["'`][^"'`]*["'`]/,
        `ownerNumber: "${deploymentUser.whatsappNumber}"`
      );
    }
  ...
  AUTO_STATUS_VIEW: "${features.AUTO_STATUS_VIEW || "off"}",
  ENABLE_STATUS_REACTION: ${safeBool(features.AUTO_REACTION)}, // Mapped
