...
async function processDailyBurn() {
  ...
  for (const bot of activeBots) {
    try {
      ...
      const burnAmount = bot.dailyBurn || 5; // Use bot specific rate or fallback to 5

      // 🔒 IDEMPOTENCY CHECK: Prevent double-billing for same day
      const lastBurnDateStr = bot.lastBurnDate
        ? new Date(bot.lastBurnDate).toISOString().split("T")[0]
        : null;

      if (lastBurnDateStr === billingPeriod) {
        console.log(
          `[Credits] Skipping ${bot.botName} - already billed for ${billingPeriod}`,
        );
        skipped++;
        continue;
      }
      ...
      if (user.credits >= burnAmount) {
        ...
        await deductCredits(
          ...
          burnAmount,
          "daily_renewal",
          `Daily renewal for bot: ${bot.botName} (Period: ${billingPeriod})`,
          { deployment: bot._id, billingPeriod },
        );

        // Update timestamps with idempotency markers
        bot.lastRenewedAt = now;
        bot.lastBurnDate = now; // 🔒 SET IDEMPOTENCY MARKER

        // 🕒 RENEWAL DRIFT FIX: Base nextRenewalAt on the intended time, not the execution time
        const oneDayMs = 24 * 60 * 60 * 1000;
        const currentNextRenewal = bot.nextRenewalAt
          ? new Date(bot.nextRenewalAt).getTime()
          : 0;

        // If current renewal is reasonable (within 48h of now), increment it exactly.
        // This prevents double billing if run twice quickly, but keeps it on track if run late.
        if (currentNextRenewal > now.getTime() - oneDayMs * 2) {
          bot.nextRenewalAt = new Date(currentNextRenewal + oneDayMs);
        } else {
          bot.nextRenewalAt = new Date(now.getTime() + oneDayMs);
        }
        ...
        // ✅ ADDED: Notification for daily deduction
        await Notification.create({
          user: user._id,
          title: "Daily Renewal 💸",
          message: `Deducted ${burnAmount} credits for daily renewal of "${bot.botName}". Remaining balance: ${Math.round(user.credits)} credits.`,
          type: "info",
        });
        ...
        bot.status = "suspended";
        bot.billingStatus = "suspended";
        bot.lastBurnDate = now; // Mark attempt even on suspend to prevent retry loops
        await bot.save();
