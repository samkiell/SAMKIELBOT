...
exports.initializePayment = async (data) => {
  try {
    ...
    const payload = {
      ...
      currency: currency,
      payment_options:
        "card,mobilemoney,ussd,banktransfer,account,credit,mpesa,qr,barter,mobilemoneyghana,mobilemoneyuganda,mobilemoneyrwanda,mobilemoneyzambia,mobilemoneytanzania",
      redirect_url:
        redirect_url ||
        `${(
          process.env.NEXT_PUBLIC_APP_URL || "https://samkielbot.app"
        ).replace(/\/$/, "")}/checkout/verify?provider=flutterwave`,
      customer: {
        email: email,
      ...
      customizations: {
        ...
        logo: "https://samkielbot.app/logo.png",
      },
    };
