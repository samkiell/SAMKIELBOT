...
const processDeployment = async (deploymentId) => {
  ...
  try {
    ...
    // Helper functions for safe injection
    const safeMsg = (str) => (str ? str.toString().replace(/"/g, '\\"') : "");
    const safeBool = (val) => (val ? "true" : "false");

    // Logic for owner number (preserved)
    ...
    // Determine Auto Status View logic for template
    const autoStatusVal =
      features.AUTO_STATUS_VIEW === "msg"
        ? "on"
        : features.AUTO_STATUS_VIEW || "off";

    // New Settings.js Content
    const modifiedSettings = `const settings = {
  // Core Foundation
  botName: "${safeMsg(deployment.botName)}",          // e.g., "My Awesome Bot"
  prefix: "${safeMsg(config.prefix || ".")}",            // e.g., "."
  botNumber: "${safeMsg(deployment.botNumber)}",     // e.g., "2348012345678"
  ownerNumber: "${safeMsg(ownerNumToInject)}", // e.g., "2348087357158"
  ownerName: "${safeMsg(
    config.ownerName || "SAMKIEL BOT"
  )}",     // e.g., "Admin 𝕊𝔸𝕄𝕂𝕀𝔼𝕃 𝔹𝕆𝕋"
  developer: "ѕαмкιєℓ.∂єν",        // Static value
  portfolio: "https://samkiel.dev",
  website: "https://samkielbot.app",
  // Feature Toggles
  featureToggles: {
    // Privacy & Security
    COMMAND_MODE: "${safeMsg(
      features.COMMAND_MODE || "public"
    )}",          // "public" or "private"
    REJECT_CALL: ${safeBool(
      features.REJECT_CALL
    )},              // Boolean: true/false
    ANTI_DELETE: ${safeBool(
      features.ANTI_DELETE
    )},              // Boolean: true/false
    ANTI_DELETE_TYPE: "${safeMsg(
      features.ANTI_DELETE_TYPE || "all"
    )}", // "all" or "dm"
    // Activity & Automation
    ALWAYS_ONLINE: ${safeBool(
      features.ALWAYS_ONLINE
    )},          // Boolean: true/false
    SEND_READ: ${safeBool(
      features.SEND_READ
    )},                  // Boolean: true/false
    AUTO_STATUS_VIEW: "${safeMsg(autoStatusVal)}",  // "on" or "off"
    STATUS_VIEW_EMOJI: "${safeMsg(
      features.STATUS_VIEW_EMOJI || "👀"
    )}",// e.g., "👀"
    STATUS_VIEW_MSG: "off",                    // Default to off
    PERSONAL_MESSAGE: ${safeBool(
      features.PERSONAL_MESSAGE
    )},    // Boolean: true/false
    DISABLE_START_MESSAGE: ${safeBool(
      features.DISABLE_START_MESSAGE
    )},  // Boolean: true/false
    RANKING: ${safeBool(
      features.RANKING
    )},                      // Boolean: true/false
    AUTO_REACTION: ${safeBool(
      features.AUTO_REACTION
    )},          // Boolean: true/false
    PACKNAME: "${safeMsg(
      config.packName || "𝕊𝔸𝕄𝕂𝕀𝔼𝕃 𝔹𝕆𝕋"
    )}",                  // e.g., "𝕊𝔸𝕄𝕂𝕀𝔼𝕃 𝔹𝕆𝕋"
    ENABLE_STATUS_REACTION: false,             // Force false/internal
  },
  // Metadata
  author: "𝕊𝔸𝕄𝕂𝕀𝔼𝕃 𝔹𝕆𝕋",                   // e.g., "𝕊𝔸𝕄𝕂𝕀𝔼𝕃 𝔹𝕆𝕋"
