...
const sendEmailBroadcast = async ({
  ...
  try {
    ...
    const pendingEmails = allEmails.filter(
      (email) => !alreadySentSet.has(email),
    );
    ...
    console.log(
      `[EmailBroadcast] Resuming/Starting: ${pendingEmails.length} emails pending out of ${totalRecipients}`,
    );

    for (let i = 0; i < pendingEmails.length; i += effectiveBatchSize) {
      const batchEmails = pendingEmails.slice(i, i + effectiveBatchSize);
      const batchNumber = Math.floor(i / effectiveBatchSize) + 1;
      const totalBatches = Math.ceil(pendingEmails.length / effectiveBatchSize);
      ...
      if (i + effectiveBatchSize < pendingEmails.length) {
...
const sendTestEmail = async ({
  ...
  const result = await sendSingleEmail(transporter, to, emailContent);
  if (!result.success) {
