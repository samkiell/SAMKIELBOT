...
const processDeployment = async (deploymentId) => {
  ...
  try {
    ...
    // Prefer the one in configuration snapshot (which includes the fallback logic we ran earlier)
    const ownerNumToInject = config.ownerNumber || deployment.user; // Fallback to user ID if somehow missing
