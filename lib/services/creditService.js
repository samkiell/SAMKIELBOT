...
async function processDailyBurn() {
  ...
  const activeBots = await Deployment.find({
    status: { $in: ["active", "running", "connected", "paired", "starting", "online", "degraded", "suspension"] }, // Check suspended too? No, only active bots consume credits basically. BUT wait, requirements say renewal is recurring.
