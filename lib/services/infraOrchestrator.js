...
class InfrastructureOrchestrator extends EventEmitter {
  ...
  _log(msg) {
    ...
    try {
      fs.appendFileSync(
        path.resolve(process.cwd(), "logs/infra-debug.log"),
        line
      );
    } catch (e) {}
    console.log(msg);
  ...
  async start() {
    this._log("[InfraOrchestrator] Starting infrastructure monitoring...");
  ...
  async poll() {
    try {
      ...
      if (!dropletId) {
        this._log("[InfraOrchestrator] Missing DIGITALOCEAN_DROPLET_ID");
        return;
      }

      this._log(`[InfraOrchestrator] Polling droplet ${dropletId}...`);
      ...
      try {
        ...
        if (metricsErr.response?.status === 404) {
          this._log(`[InfraOrchestrator] Metrics 404 for ${info.name}.`);
        } else {
          this._log(`[InfraOrchestrator] Metrics Error: ${metricsErr.message}`);
        }
      }

      // 4. Get Per-Bot Metrics (Attribution)
      this._log("[InfraOrchestrator] Fetching per-bot metrics...");
      const botStats = await this.fetchBotMetrics();
      this._log(`[InfraOrchestrator] Fetched ${botStats.length} bot stats.`);

      // 5. Normalize and Store
      const liveState = this.normalizeMetrics(metrics, info, botStats);

      // 6. Alerting & Prediction
      this._log("[InfraOrchestrator] Running alerting and prediction...");
      ...
      this._log("[InfraOrchestrator] Storing live state in cache...");
      ...
      this._log(`[InfraOrchestrator] Data stored in Cache: ${storeStatus}`);

      // 8. Update DB (Local state management)
      try {
        await this.updateNodeStats(liveState, info);
        this._log("[InfraOrchestrator] Node stats updated in DB.");
      } catch (dbErr) {
        this._log(`[InfraOrchestrator] DB Update Error: ${dbErr.message}`);
      }

      this._log(`[InfraOrchestrator] Emitting update for ${liveState.name}`);
