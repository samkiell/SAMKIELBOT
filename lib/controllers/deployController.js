...
const deployBot = async (req, res) => {
  return errorResponse(
    res,
    "We're currently on maintenance. Deployment is temporarily on hold.",
    503,
  );
};
...
const controlServer = async (req, res) => {
  try {
    ...
    if (signal === "start" || signal === "restart") {
      ...
      // ✅ Reactivation Fix: Reset billingStatus if bot was suspended
      if (deployment.billingStatus === "suspended") {
        const userDoc = await User.findById(req.user.id);
        const requiredCredits = deployment.dailyBurn || 5;

        if (userDoc.credits < requiredCredits) {
          return errorResponse(
            res,
            `Insufficient credits to reactivate bot. You need at least ${requiredCredits} credits.`,
            403,
          );
        }

        updatePayload.billingStatus = "active";
        console.log(
          `[Billing] Reactivating bot ${deployment._id} for user ${req.user.id}`,
        );
      }
    }
...
const processDeployment = async (deploymentId) => {
  ...
  featureToggles: {
    ...
    ENABLE_STATUS_REACTION: ${safeBool(features.ENABLE_STATUS_REACTION !== false)},
    ANTI_DELETE: ${safeBool(features.ANTI_DELETE !== false)},
    SEND_READ: ${safeBool(features.SEND_READ)},
    ALWAYS_ONLINE: ${safeBool(features.ALWAYS_ONLINE !== false)},
    ...
    COMMAND_MODE: "${safeMsg(features.COMMAND_MODE || "private")}",
    VOICE_CHAT: ${safeBool(features.VOICE_CHAT)},
    PACKNAME: "${safeMsg(config.packName || "𝕊𝔸𝕄𝕂𝕀𝔼𝕃 𝔹𝕆𝕋")}",
    AUTO_RESTART: ${safeBool(features.AUTO_RESTART !== false)}
  },
