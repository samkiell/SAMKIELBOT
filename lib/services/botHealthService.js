...
class BotHealthService extends EventEmitter {
  ...
  async connectWebSocket(monitor) {
    try {
      ...
      console.log(
        `[BotHealth] Raw message for ${monitor.identifier}: ${msgStr.substring(
          0,
          100
        )}`
      );
  ...
  async performHealthCheck(deploymentId) {
    try {
      ...
      const updateFields = {
        ...
        lastHeartbeatAt: new Date(), // Mark successful pull from Ptero as a heartbeat
        lastActiveAt: new Date(),
      };
      ...
      if (
        ...
        updateFields["restartCount"] = (deployment.restartCount || 0) + 1000; // Increment. Wait, +1 makes more sense.
