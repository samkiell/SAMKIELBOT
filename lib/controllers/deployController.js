...
module.exports = settings;`;
    ...
    try {
      ...
      // We wait for installation to ensure the server has already cloned the branch
      try {
        const branchName = `bot-${deployment.botNumber}`;
        console.log(`[Git] Deleting deployment branch: ${branchName}`);
        await octokit.git.deleteRef({
          owner,
          repo,
          ref: `heads/${branchName}`,
        ...
        // We don't fail the deployment if branch deletion fails
      }
    } catch (startError) {
      console.error(
