 ...
 * @param {string} [params.callbackUrl] - Optional custom callback URL
...
async function initializePayment({
  ...
  try {
    const response = await paystackAxios.post("/transaction/initialize", {
      email,
      amount: Math.round(amount * 100), // Convert to smallest unit (kobo/cents)
      currency: currency.toUpperCase(),
      reference,
      metadata,
      callback_url:
        callbackUrl ||
        `${process.env.FRONTEND_URL}/dashboard?payment=success&reference=${reference}`,
    });
