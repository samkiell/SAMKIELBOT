...
const createServer = async (deploymentData) => {
  try {
    ...
    const payload = {
      ...
      startup:
        "if [ -f .setup_done ]; then echo 'Updating existing installation from main...'; if [ -f settings.js ]; then cp settings.js settings.js.bak; fi; git fetch origin main && git reset --hard origin/main; if [ -f settings.js.bak ]; then mv settings.js.bak settings.js; fi; else echo 'First-time deployment, setup from branch $BRANCH...'; if [ ! -d .git ]; then git init . && git remote add origin $REPO_URL; fi; git fetch origin $BRANCH && git reset --hard origin/$BRANCH && touch .setup_done; fi; npm install; node index.js",
      environment: {
