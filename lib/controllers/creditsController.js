...
exports.getCreditBalance = async (req, res) => {
  try {
    ...
    // Calculate UTC+1 Start of Today (Midnight UTC+1 = 23:00 UTC previous day)
    const now = new Date();
    // To get the current day in UTC+1, we offset by +1 hour
    const utcPlusOneTime = new Date(now.getTime() + 1 * 60 * 60 * 1000);
    const startOfTodayUTC = new Date(
      Date.UTC(
        utcPlusOneTime.getUTCFullYear(),
        utcPlusOneTime.getUTCMonth(),
        utcPlusOneTime.getUTCDate()
      ) -
        1 * 60 * 60 * 1000 // Subtract 1h to reach 23:00 UTC (Midnight UTC+1)
    );
    ...
    res.json({
      ...
      data: {
        ...
        dailyClaim: {
          canClaim: (() => {
            // Allow claim if never claimed OR if last claim was before today's 00:00 UTC-1
            const eligibility =
              !user.lastDailyClaim ||
...
exports.claimDailyCredits = async (req, res) => {
  try {
    ...
    // 2. Calculate UTC-1 Start of Today (Midnight UTC-1 = 01:00 UTC)
    const now = new Date();
    const utcMinusOneTime = new Date(now.getTime() - 1 * 60 * 60 * 1000);
    const startOfTodayUTC = new Date(
      Date.UTC(
        utcMinusOneTime.getUTCFullYear(),
        utcMinusOneTime.getUTCMonth(),
        utcMinusOneTime.getUTCDate()
      ) +
        1 * 60 * 60 * 1000
    );
    ...
    if (!lastClaimDate) {
      ...
      // Use the same UTC-1 offset for the last claim date to find its "Start of Day"
      const lastClaimOffset = new Date(
        lastClaimDate.getTime() - 1 * 60 * 60 * 1000
      );
      const lastClaimStartOfToday = new Date(
        Date.UTC(
          lastClaimOffset.getUTCFullYear(),
          lastClaimOffset.getUTCMonth(),
          lastClaimOffset.getUTCDate()
        ) +
          1 * 60 * 60 * 1000
      );
