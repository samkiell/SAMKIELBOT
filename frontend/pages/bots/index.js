...
export default function BotsList() {
  ...
  const fetchBots = async () => {
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/bots-list`);
      const data = await res.json();
      
      // Add uptimeStart to each bot for real-time calculation
      const botsWithUptime = (data.data || []).map(bot => ({
        ...bot,
        uptimeStart: bot.isActive && bot.lastActive 
          ? new Date(bot.lastActive).getTime() - (bot.uptime * 60000)
          : null
      }));
      
      setBots(botsWithUptime);
      setFilteredBots(botsWithUptime);
    } catch (err) {
