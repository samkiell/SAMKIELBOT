...
class InfrastructureOrchestrator extends EventEmitter {
  ...
  normalizeMetrics(metrics, info, botStats = []) {
    ...
    if (metrics) {
      ...
      // Pterodactyl Fallback: If DO metrics are offline, sum up the bots
      // This ensures the dashboard always shows the workload even without DO Agent
      ...
      // Note: This won't include system overhead, but it's better than 0%
    }
    ...
    return {
      ...
      host: {
        cpu: {
          usedPercent: Math.min(100, Math.round(cpuUsage * 100) / 100),
          cores: info.vcpus,
        },
        memory: {
