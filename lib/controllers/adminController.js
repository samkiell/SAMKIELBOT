...
const sendEmailBroadcast = async (req, res) => {
  try {
    ...
    await logAdminAction(
      ...
      `Admin initiated email broadcast: "${subject.trim().substring(0, 50)}..."`
    );
    ...
    await logAdminAction(
      req.user._id,
      "email_broadcast_completed",
      `Email broadcast completed: ${result.stats.sent}/${result.stats.totalRecipients} sent`
    );

    successResponse(res, result, "Email broadcast sent successfully");
  } catch (error) {
...
// @desc    Send Test Email (preview before broadcast)
// @route   POST /api/admin/email-broadcast/test
// @access  Admin only
const sendTestEmailBroadcast = async (req, res) => {
  try {
    const { testEmail, subject, message, announcementType, priority } =
      req.body;
    ...
    // Send the test email
    const result = await emailBroadcastService.sendTestEmail({
