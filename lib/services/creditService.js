...
async function processDailyBurn() {
  ...
  const activeBots = await Deployment.find({
    ...
    $or: [{ lastBurnDate: { $lt: todayUTC } }, { lastBurnDate: null }],
  }).populate("user");

  console.log(
    `[Credits] Running Daily Burn for ${
      activeBots.length
    } bots (Cycle: ${todayUTC.toISOString()})`
  );
  ...
  for (const bot of activeBots) {
    try {
      ...
      const burnAmount = bot.dailyBurn || 5; // Default if missing
      ...
      if (user.credits >= burnAmount) {
        ...
        await Notification.create({
          user: user._id,
          title: "Bot Suspended 🔴",
          message: `Your bot "${bot.botName}" was suspended due to insufficient credits. Please claim daily rewards or purchase credits to reactivate.`,
          type: "error",
        });
      }
    } catch (error) {
