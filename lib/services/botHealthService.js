...
class BotHealthService extends EventEmitter {
  ...
  async parseLogForStateChange(deploymentId, logLine) {
    ...
    // 3. ACTIVE (Verified Connection) (Requirement 4)
    ...
    if (
      ...
      console.log(`[BotHealth] Bot ACTIVE: ${deployment.botName}`);
      ...
      // Send success notification (Requirement 4) - Only if coming from non-active state
      if (
        !["active", "online", "connected", "degraded"].includes(
          deployment.status
        )
      ) {
  ...
  async performHealthCheck(deploymentId) {
    try {
      ...
      // Fix: Auto-recover from "stuck in starting" if server is running and up for > 2 mins
      if (
        ...
        console.log(
          `[BotHealth] Auto-correcting status for ${deployment.botName}: Starting -> Online (Uptime: ${uptimeMinutes}m)`
        );
        this.transitionState(deploymentId, "online", {
          isActive: true, // Force active
        });
        // We update the local variable effectively by not returning,
        // but transitionState is async and does the DB write.
        // We can just let the next cycle pick it up or assume it's done.
      }
  ...
  // Force update status (mostly for auto-detection)
  async forceUpdateStatus(deploymentId, status) {
    await this.transitionState(deploymentId, status, {
      isActive: ["active", "online", "connected"].includes(status),
    });
  }
  ...
  async sendSuccessNotification(deployment) {
    try {
      await Notification.create({
        ...
        message: `Your bot "${deployment.botName}" is now active and connected.`,
        type: "success",
      });
    } catch (error) {
