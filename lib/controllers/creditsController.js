      ...
      data: {
        ...
        dailyClaim: {
          canClaim: (() => {
            ...
            // Add a small 5-second grace period to handle clock drift between client and server
            const gracePeriodMs = 5000;
            const eligibility =
              !user.lastDailyClaim ||
              new Date(user.lastDailyClaim).getTime() <
                startOfTodayUTC.getTime() - gracePeriodMs;
...
exports.claimDailyCredits = async (req, res) => {
  try {
    ...
    // 1. Calculate UTC Start of Today (00:00:00.000 UTC)
    const now = new Date();
    const startOfTodayUTC = new Date(
      Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate())
    );

    // 2. Calculate Next Claim Time (Tomorrow 00:00:00 UTC)
    const nextClaimTime = new Date(startOfTodayUTC);
    nextClaimTime.setUTCDate(nextClaimTime.getUTCDate() + 1);

    // 3. Attempt Atomic Update
    // We strictly look for a user where lastDailyClaim is BEFORE today's 00:00 UTC
    // OR lastDailyClaim is null.
    ...
    console.error("[Credits] Daily claim error:", error);
    res.status(500).json({
      success: false,
