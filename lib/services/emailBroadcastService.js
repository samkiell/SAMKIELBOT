...
const sendEmailBroadcast = async ({
  ...
  try {
    ...
    for (let i = 0; i < pendingRecipients.length; i += effectiveBatchSize) {
      const batchRecipients = pendingRecipients.slice(i, i + effectiveBatchSize);
      const batchNumber = Math.floor(i / effectiveBatchSize) + 1;
      const totalBatches = Math.ceil(pendingRecipients.length / effectiveBatchSize);
...
const sendTestEmail = async ({
  ...
  const result = await sendSingleEmail(transporter, to, emailContent, "Test User");
  if (!result.success) {
