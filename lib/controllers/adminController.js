...
const sendEmailBroadcast = async (req, res) => {
  try {
    ...
    const result = await emailBroadcastService.sendEmailBroadcast({
      ...
      senderName: req.body.senderName, // Pass sender name from request
      subject: subject.trim(),
      message: message.trim(), // Use as HTML
    ...
    successResponse(res, result, "Email broadcast queued successfully");
  } catch (error) {
...
// @desc    Send Test Email Broadcast (Single Recipient)
...
const sendTestEmailBroadcast = async (req, res) => {
  try {
    ...
    await emailBroadcastService.sendTestEmail({
      to: testEmail,
      senderName,
      subject,
      message,
...
const resumeEmailBroadcast = async (req, res) => {
  try {
    ...
    const result = await emailBroadcastService.resumeBroadcast(broadcastId);

    // Log completion
    await logAdminAction(
      req,
      "email_broadcast_resumed",
      {
        type: "EmailBroadcast",
        id: broadcastId,
