  ...
  try {
    ...
    if (req.body.quoteId) {
      ...
      const paymentTransaction = await PaymentTransaction.create({
        user: userId,
        reference,
        amount: quote.paystackChargeAmount, // Charge the NGN amount
        currency: quote.processingCurrency, // Should be "NGN"
      ...
      const paymentResult = await paystackService.initializePayment({
        ...
        currency: quote.processingCurrency, // "NGN"
        reference,
        metadata: {
...
exports.createQuote = async (req, res) => {
  try {
    ...
    // FORCE NGN: Paystack doesn't support international currencies for this merchant account
    const processingCurrency = "NGN"; // Always NGN

    // Get Rate (For display purposes only)
    const rates = await exchangeRateService.getExchangeRates();
    const desiredCurrency = currency || "NGN";
    // rate should be NGN -> SelectedCurrency
    const rate = rates[desiredCurrency] || rates["USD"];

    // Calculate Amounts (Display)
    ...
    // Calculate Actual Charge (NGN)
    // Base NGN + 8.5% Tax
    const taxAmountNgn = basePriceNgn * taxRate;
    const paystackChargeAmount = basePriceNgn + taxAmountNgn;
    ...
    const quote = await PaymentQuote.create({
      ...
      selectedCurrency: desiredCurrency, // User's preference
      processingCurrency: processingCurrency, // Actual charge currency (NGN)
      exchangeRate: rate,
