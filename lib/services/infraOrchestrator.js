...
const InfrastructureState = require("../../models/InfrastructureState");
...
class InfrastructureOrchestrator extends EventEmitter {
  constructor() {
    ...
    this.liveState = null; // Memory fallback
  }
  ...
  async poll() {
    try {
      ...
      try {
        ...
        console.log(
          `[InfraOrchestrator] Metrics fetched: ${metrics ? "YES" : "NO"}`
        );
      } catch (metricsErr) {
        if (metricsErr.response?.status === 404) {
          console.log(`[InfraOrchestrator] Metrics 404 for ${info.name}.`);
        } else {
          console.log(
            `[InfraOrchestrator] Metrics Error: ${metricsErr.message}`
          );
        }
      }

      // 4. Get Per-Bot Metrics (Attribution)
      console.log("[InfraOrchestrator] Fetching per-bot metrics...");
      const botStats = await this.fetchBotMetrics();
      console.log(`[InfraOrchestrator] Fetched ${botStats.length} bot stats.`);
      ...
      console.log("[InfraOrchestrator] Running alerting and prediction...");
      ...
      console.log("[InfraOrchestrator] Storing live state in cache...");
      const storeStatus = await redis.set(
        this.cacheKey,
        JSON.stringify(liveState),
        "EX",
        120
      );
      console.log(`[InfraOrchestrator] Data stored in Cache: ${storeStatus}`);

      // PERSIST TO MONGODB (Required for Vercel/Serverless support)
      try {
        await InfrastructureState.findOneAndUpdate(
          { key: "live_state" },
          { data: liveState, lastUpdated: new Date() },
          { upsert: true }
        );
        console.log("[InfraOrchestrator] State persisted to MongoDB.");
      } catch (persistErr) {
        console.error("[InfraOrchestrator] MongoDB persist error:", persistErr);
      }

      // 8. Update DB (Local state management)
      try {
        await this.updateNodeStats(liveState, info);
        console.log("[InfraOrchestrator] Node stats updated in DB.");
      } catch (dbErr) {
        console.log(`[InfraOrchestrator] DB Update Error: ${dbErr.message}`);
      }

      console.log(`[InfraOrchestrator] Emitting update for ${liveState.name}`);
      this.emit("infra.update", liveState);
    } catch (error) {
      console.log(
        `[InfraOrchestrator] Poll Error: ${error.stack || error.message}`
      );
    }
  }
  ...
  normalizeMetrics(metrics, info, botStats = []) {
    ...
    const memAvailable = getLatestValue(metrics?.memory);
    const fsFree = getLatestValue(metrics?.filesystem);

    // Only proceed with DO metrics if we have at least CPU or RAM data
    if (metrics && (idlePercent !== null || memAvailable !== null)) {
      // 1. CPU
      if (metrics.cpu && metrics.cpu.length > 0) {
        // DigitalOcean metrics can be raw cumulative seconds (counters)
        // Ratio = (Total - Idle) / Total
        const totalCpuSeconds = metrics.cpu.reduce((acc, s) => {
          const v = s.values?.slice(-1)[0]?.[1];
          return acc + (v ? parseFloat(v) : 0);
        }, 0);

        const idleSeconds = getLatestValue(metrics.cpu, { mode: "idle" });

        if (totalCpuSeconds > 0 && idleSeconds !== null) {
          cpuUsage = ((totalCpuSeconds - idleSeconds) / totalCpuSeconds) * 100;
        } else if (idlePercent !== null) {
          // Fallback to percentage-based if it looks like one (0-100)
      ...
      if (memAvailable !== null) {
        if (
          ...
          usedMemoryBytes = Math.max(0, totalMemoryBytes - memAvailable);
        } else {
          usedMemoryBytes = memAvailable;
        }
      }

      // 3. Disk
      if (fsFree !== null) {
        usedDiskBytes = Math.max(0, totalDiskBytes - fsFree);
      }

      console.log(
        `[InfraOrchestrator] DO Stats: CPU=${cpuUsage.toFixed(1)}%, RAM=${(
          usedMemoryBytes /
          1024 /
          1024
        ).toFixed(0)}MB`
      ...
      console.log(
        `[InfraOrchestrator] Falling back to Ptero stats (DO data restricted/lagging)`
      );
    }
    ...
    const uptimeMs = info.created_at
      ? Date.now() - new Date(info.created_at).getTime()
      : 0;
    ...
    return {
      ...
      dropletId: info.id,
      name: info.name,
      status: info.status === "active" ? "online" : info.status,
      uptimeMinutes, // Droplet total uptime
      host: {
        cpu: {
          usedPercent: Math.min(100, Math.round(cpuUsage * 10) / 10) || 0,
          cores: info.vcpus || 1,
        },
        memory: {
  ...
  async fetchBotMetrics() {
    try {
      ...
      return deployments.map((d) => ({
        ...
        limitCpu: d.resources?.cpuLimit || 0,
        limitDisk: d.resources?.diskLimit || 0,
  ...
  async getLiveState() {
    try {
      ...
      // 2. Memory Fallback (If orchestrator is running in same process)
      if (this.liveState) return this.liveState;

      // 3. Database Fallback (For Vercel/Serverless)
      const doc = await InfrastructureState.findOne({ key: "live_state" });
      if (doc && doc.data) {
        console.log(
          "[InfraOrchestrator] Cache miss, loaded state from MongoDB"
        );
        return doc.data;
      }
      ...
      console.error("[InfraOrchestrator] getLiveState error:", err);
