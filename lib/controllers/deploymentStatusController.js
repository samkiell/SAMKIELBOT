...
const getDeploymentStatus = async (req, res) => {
  try {
    ...
    // Lazy Sync: If bot is in a state where we expect progress but have no pairing code,
    // try to force a one-off sync from the logs. This helps on serverless builds (Vercel).
    if (
      ["starting", "creating", "installing"].includes(deployment.status) &&
      !deployment.pairingCode
    ) {
      // We don't 'await' it to avoid blocking the API response too long,
      // but the UI will catch it on the next poll.
      // However, for the very first visit, let's wait a bit.
